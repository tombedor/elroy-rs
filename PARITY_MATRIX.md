...
| Codex workflow support in v1 | `partial` | `crates/elroy-codex/src/lib.rs`, `crates/elroy-app/src/lib.rs` | Persisted session metadata, async dispatch/resume lifecycle, isolated worktree handling, tool-surface inspection, and recursive session-file discovery now exist, but assistant follow-up behavior and broader UI workflow support are still missing |
