...
| `elroy/messenger/` | stream/event messaging loop | `crates/elroy-app` and `crates/elroy-core` | `partial` | `crates/elroy-app/src/lib.rs`, `crates/elroy-core/src/lib.rs` | A messenger-style shared runtime surface now exists via `AppRuntime::process_message`, which wraps the core orchestrator path, supports user/system role persistence, tool enable/disable, and `persist_input_message=false`, and is reused for both foreground prompts and background Codex follow-up turns; the explicit Python force-tool surface and iterator-shaped streaming API are still missing |
