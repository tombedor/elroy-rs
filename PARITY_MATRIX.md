...
| `elroy/config/` | env/config-file/model config resolution | `crates/elroy-config` | `partial` | `crates/elroy-config/src/lib.rs` | Defaults, YAML loading, and env override precedence exist; model/config surface is still narrow |
| `elroy/core/` | session, turn, runtime, orchestration | `crates/elroy-core` | `partial` | `crates/elroy-core/src/lib.rs` | Session/turn boundaries exist, the orchestrator accumulates both stream events and normalized transcript messages, and a live provider adapter can drive real provider calls; higher-level repository composition is still missing |
...
| `elroy/repository/tasks/` | task mutation workflows | `crates/elroy-tasks` | `not started` | none | Confirm exact role during port |
| `elroy/repository/user/` | user session and preferences | `crates/elroy-user` | `not started` | none | Assistant name and user naming matter |
| `elroy/repository/codex_sessions/` | Codex session persistence and workflows | `crates/elroy-codex` | `not started` | none | Needed for current roadmap direction |
...
| Memory recall quality | `not started` | none | Needs scenario-driven tests, not just unit tests |
| Reminder and due-item surfacing | `not started` | none | Timing and contextual behavior are easy to regress |
| Codex workflow support in v1 | `not started` | none | Required for the first Rust version |
