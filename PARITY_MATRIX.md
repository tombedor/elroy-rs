...
| `elroy/messenger/` | stream/event messaging loop | `crates/elroy-app` and `crates/elroy-core` | `partial` | `crates/elroy-app/src/lib.rs`, `crates/elroy-core/src/lib.rs`, `crates/elroy-llm/src/lib.rs` | A messenger-style shared runtime surface now exists via `AppRuntime::process_message`, which wraps the core orchestrator path, supports user/system role persistence, tool enable/disable, `persist_input_message=false`, and named `force_tool` selection that is forwarded into provider-specific tool-choice requests; it is reused for both foreground prompts and background Codex follow-up turns. The remaining gap is the Python iterator-shaped streaming API rather than role or force-tool coverage |
