...
| `elroy/messenger/` | stream/event messaging loop | `crates/elroy-app` and `crates/elroy-core` | `partial` | `crates/elroy-app/src/lib.rs`, `crates/elroy-core/src/lib.rs`, `crates/elroy-llm/src/lib.rs` | A messenger-style shared runtime surface now exists via `AppRuntime::process_message` and buffered `process_message_stream`, which wrap the core orchestrator path, support user/system role persistence, tool enable/disable, `persist_input_message=false`, and named `force_tool` selection that is forwarded into provider-specific tool-choice requests; the same path is reused for both foreground prompts and background Codex follow-up turns. The remaining gap is true incremental streaming semantics rather than missing role, force-tool, or iterator-shaped surface area |
