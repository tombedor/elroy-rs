...
- `elroy-app`: shared runtime behavior for provider config translation, snapshot loading, prompt execution, startup restart/greeting stream handling, prompt-time memory recall classifier/window controls plus recall-status prelude behavior, DB-backed sidebar-detail loading, and file-backed memory/agenda mutation tools
