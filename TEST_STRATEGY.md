...
- `elroy-app`: shared runtime behavior for provider config translation, snapshot loading, prompt execution, startup restart/greeting stream handling, prompt-time memory recall classifier/window controls, DB-backed sidebar-detail loading, and file-backed memory/agenda mutation tools
