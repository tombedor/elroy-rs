...
- `elroy-core`: session and turn boundaries plus provider-neutral conversation/tool-loop orchestration with normalized transcript accumulation and live-provider runtime wiring
...
- `elroy-llm`: stream event model, partial tool-call accumulation, validated context-message rules, provider request payload building, live HTTP client request shaping, and provider response parsing
