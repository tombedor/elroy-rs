...
- `elroy-app` + `elroy-db` + `elroy-agenda`: active/inactive due-item query behavior, Python-style timed-due prompt surfacing including multiple simultaneous due items plus app-level prompt/runtime handling and cleanup through `delete_due_item`, heuristic contextual-trigger surfacing, Python-named active due-item inspection plus active/inactive listing aliases, and file-backed due-item create/update/rename/complete/delete flows including delete-then-recreate same-name reminder reuse
