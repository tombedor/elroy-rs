...
- `elroy-app`: shared runtime behavior for provider config translation, snapshot loading, prompt execution, startup restart/greeting stream handling, Python-style prompt prelude status ordering (`loading context...`, recall status, `thinking...`), prompt-time memory recall classifier/window controls, DB-backed sidebar-detail loading, and file-backed memory/agenda mutation tools
- `elroy-core`: shared turn orchestration, validated transcript repair, force-tool forwarding, and Python-style tool-loop status ordering around local tool execution in both buffered and streamed paths
