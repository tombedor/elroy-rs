...
use elroy_db::{UserPreferenceRecord, load_context_messages, load_user_preferences, replace_context_messages};
...
use crate::{AppError, LOCAL_USER_TOKEN, SYNTHETIC_FIRST_USER_MESSAGE, excerpt, fast_provider_config_from_app_config};

const CONTEXT_REFRESH_SUMMARY_WORD_LIMIT: usize = 300;

pub(crate) fn drop_old_context_messages(
...
pub(crate) fn current_system_message(
...
pub(crate) fn repair_system_message_placement(
...
pub(crate) fn repair_first_user_precedes_first_assistant(
    raw_messages: &[ConversationMessage],
...
pub(crate) fn load_validated_runtime_transcript(
    connection: &mut rusqlite::Connection,
    default_assistant_name: &str,
    ensure_alternating_roles: bool,
...
pub(crate) fn refreshed_context_messages_with_system(
    connection: &mut rusqlite::Connection,
    config: &AppConfig,
) -> Result<Vec<ConversationMessage>, AppError> {
    let preferences = load_user_preferences(connection, LOCAL_USER_TOKEN)?;
...
pub(crate) fn refresh_persisted_system_instructions(
    connection: &mut rusqlite::Connection,
    config: &AppConfig,
) -> Result<(), AppError> {
...
pub(crate) fn reset_persisted_context(
    connection: &mut rusqlite::Connection,
    config: &AppConfig,
) -> Result<(), AppError> {
    let preferences = load_user_preferences(connection, LOCAL_USER_TOKEN)?;
...
pub(crate) fn context_refresh_summary_system_prompt(assistant_name: &str) -> String {
    format!(
...
pub(crate) fn format_context_messages_for_summary(
...
pub(crate) fn summarize_context_messages_with_model(
...
pub(crate) fn build_context_summary_message(
...
pub(crate) fn format_context_summary_message(messages: &[ConversationMessage]) -> String {
    let lines = messages
        .iter()
        .filter_map(|message| {
...
pub(crate) fn format_context_summary_timestamp(unix_seconds: i64) -> String {
    Local
...
pub(crate) fn approximate_message_token_count(message: &ConversationMessage) -> usize {
...
pub(crate) fn count_context_tokens(context_messages: &[ConversationMessage]) -> usize {
    context_messages
        .iter()
...
pub(crate) fn is_context_refresh_needed(context_messages: &[ConversationMessage], max_tokens: usize) -> bool {
    if !context_messages
        .iter()
...
pub(crate) fn compress_context_messages(
...
pub(crate) fn strip_transient_context_messages(
...
pub(crate) fn strip_input_message_for_persistence(
    mut transcript: Vec<ConversationMessage>,
    persistent_prefix_len: usize,
