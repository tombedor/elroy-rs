...
impl TuiRuntime for CliTuiRuntime {
    ...
    fn start_startup_prompt_stream(&mut self) -> Result<Option<Box<dyn TuiPromptStream>>, String> {
        let restart_resume_message = self.startup_resume_message.take();
...
mod tests {
    ...
    fn cli_tui_runtime_restart_stream_queues_and_runs_deferred_auto_memory() {
        ...
        let mut stream = runtime
            .start_restart_prompt_stream("Restarted successfully. Ready to continue.")
            .expect("restart stream should start");
        while stream
            .next_update()
    ...
    #[test]
    fn cli_tui_runtime_consumes_startup_resume_message_only_once() {
        let _guard = cli_runtime_test_lock()
            .lock()
            .expect("cli runtime test lock should work");
        let unique = format!(
            "elroy-rs-cli-startup-resume-once-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("system clock should be after unix epoch")
                .as_nanos()
        ...
        let mut runtime = CliTuiRuntime::new(AppRuntime::new(config));
        runtime.startup_resume_message =
            Some("Restarted successfully. Ready to continue.".to_string());

        let mut first_stream = runtime
            .start_startup_prompt_stream()
            .expect("first startup stream should evaluate")
            .expect("first startup stream should exist");
        while first_stream
            .next_update()
            .expect("stream should advance")
            .is_some()
        {}
        let _ = first_stream.finalize().expect("snapshot should finalize");

        assert!(
            runtime
                .start_startup_prompt_stream()
                .expect("second startup stream should evaluate")
                .is_none()
        );
    ...
    fn cli_tui_runtime_prompt_stream_queues_and_runs_deferred_auto_memory() {
        ...
        let _mock = server
            ...
            .with_body(concat!(
                "event: response.output_text.delta\n",
                "data: {\"delta\":\"Adding the missing detail now.\"}\n\n",
                "data: [DONE]\n\n"
            ))
            .create();
        ...
        let mut runtime = CliTuiRuntime::new(AppRuntime::new(config));
        let mut stream = runtime
            .start_prompt_stream("The main point was the Friday launch deadline.")
            .expect("prompt stream should start");
        while stream
            .next_update()
            .expect("stream should advance")
            .is_some()
    ...
    fn cli_tui_runtime_background_command_can_return_restart_request() {
        ...
        runtime
            .start_command_execution(TuiCommandExecution {
                command_name: "restart_session".to_string(),
                display_name: "restart_session".to_string(),
                values: vec![(
        ...
        assert_eq!(
            runtime
                ...
                .expect("restart request should load")
                .as_deref(),
    ...
    fn cli_tui_runtime_runs_deferred_self_reflection_through_polling_boundary() {
        ...
        runtime
            .run_self_reflection_if_needed()
            .expect("deferred self reflection should schedule");

        assert!(runtime.deferred_self_reflection.is_some());

        let deadline = std::time::Instant::now() + Duration::from_secs(2);
        while std::time::Instant::now() < deadline {
            let _ = runtime
                .load_snapshot()
                .expect("snapshot reload should poll deferred self reflection");
            if runtime.deferred_self_reflection.is_none() {
                break;
            }
            std::thread::sleep(Duration::from_millis(10));
        }
        ...
        assert_eq!(snapshot.improvement_titles.len(), 1);
        assert!(
            snapshot.improvement_titles[0]
    ...
    #[test]
    fn cli_tui_runtime_surfaces_resumed_codex_background_statuses_through_polling_boundary() {
        let _guard = cli_runtime_test_lock()
            .lock()
            .expect("cli runtime test lock should work");
        let unique = format!(
            "elroy-rs-cli-codex-resume-background-status-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("system clock should be after unix epoch")
                .as_nanos()
        ...
        while runtime
            .poll_command_execution()
            .expect("dispatch polling should succeed")
            .is_none()
        {
            std::thread::sleep(Duration::from_millis(5));
        }
        let _ = wait_for_runtime_background_status(
            &mut runtime,
            Some("codex session thread-123 running..."),
        );
        let _ = wait_for_runtime_background_status(
        ...
        let _ = wait_for_runtime_snapshot_with_codex_session(
            &mut runtime,
            "sample (completed) thread-123",
        );
        assert_eq!(wait_for_runtime_background_status(&mut runtime, None), None);
        ...
        runtime
            .start_command_execution(TuiCommandExecution {
                command_name: "resume_codex_session".to_string(),
                display_name: "resume_codex_session".to_string(),
                values: vec![
