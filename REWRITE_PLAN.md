...
- `M4` terminal UI shell: now consumes the shared messenger stream incrementally with editable-draft, blocked-resubmit, and `Ctrl+C` clear/cancel behavior; it also covers startup restart/greeting stream handling, Python-style rendered-message bookkeeping after bootstrap/chat turns, polled rendering of newly arrived background context-message tails, and the idle model/background-status footer, but still lacks broader mutation flows and background-status producer parity
