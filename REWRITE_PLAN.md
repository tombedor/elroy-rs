...
- `M4` terminal UI shell: now consumes the shared messenger stream incrementally with editable-draft, blocked-resubmit, `Ctrl+C` clear/cancel behavior, and startup restart/greeting stream handling, but still lacks broader bootstrap/session rendering and context-polling parity
